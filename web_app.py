...
if inp_file and obs_file:
    ...
    if st.button("Run Calibration with Persistency"):
        ...
        algorithm = NSGA2(pop_size=40, sampling=IntegerRandomSampling(), crossover=SBX(prob=0.9, eta=15), mutation=PM(eta=20), eliminate_duplicates=True)
        res = minimize(problem, algorithm, ('n_gen', 50), verbose=False)

        idx_best = np.argmin(res.F[:, 0] + res.F[:, 1])
...
if 'pareto_pressure' in st.session_state and 'pareto_flow' in st.session_state:
    st.subheader("📊 Pareto Front with Highlighted Best Solution")
    
    # Retrieve best index again for safety
    idx_best = np.argmin(st.session_state['pareto_pressure'] + st.session_state['pareto_flow'])
    
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.scatter(st.session_state['pareto_pressure'], st.session_state['pareto_flow'], c='red', edgecolors='k', label="Pareto Front")
    ax.scatter(st.session_state['pareto_pressure'][idx_best], st.session_state['pareto_flow'][idx_best],
               c='blue', edgecolors='k', s=120, label="Selected Best", marker='o')
